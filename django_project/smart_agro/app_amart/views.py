...
config = {
  "apiKey": "AIzaSyAnJ-tKtif0WtxxcSFDr9VtSi7f63X7klk",
  "authDomain": "smartagro-project.firebaseapp.com",
  "databaseURL": "https://smartagro-project-default-rtdb.firebaseio.com",
  "projectId": "smartagro-project",
  "storageBucket": "smartagro-project.appspot.com",
  "messagingSenderId": "959033524882",
  "appId": "1:959033524882:web:9814b4f365f8c1480d9c4b"
}

firebase = pyrebase.initialize_app(config)
database = firebase.database()

def recupere(request):
    humidity = database.child('data').child('humidity').get().val()
    luminosite = database.child('data').child('luminosite').get().val() # Corrected the variable name
    rainfall = database.child('data').child('rainfall').get().val()
    temperature = database.child('data').child('temperature').get().val() # Corrected the variable name
    
    return render(request, "app_amart/recupere.html", {
        "humidity": humidity,
